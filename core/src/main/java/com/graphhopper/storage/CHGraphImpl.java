...
public class CHGraphImpl implements CHGraph, Storable<CHGraph> {
	...
	CHGraphImpl(Weighting w, Directory dir, final BaseGraph baseGraph, final String type) {
		...
		this.chEdgeAccess = new EdgeAccess(shortcuts, baseGraph.bitUtil) {
			...
			final long toPointer(int shortcutId) {
				assert isInBounds(shortcutId) : "shortcutId " + shortcutId + " not in bounds [" + baseGraph.edgeCount
						+ ", " + (baseGraph.edgeCount + shortcutCount) + ")";
			...
			final long reverseFlags(long edgePointer, long flags, int encoderIndex) {
				...
				if (!isShortcut)
					return baseGraph.edgeAccess.reverseFlags(edgePointer, flags, encoderIndex);
	...
	void _freeze() {
		...
		for (long pointer = N_CH_REF, basePointer = baseGraph.N_EDGE_REF; pointer < maxCapacity; pointer += nodeCHEntryBytes, basePointer += baseGraph.nodeEntryBytes) {
			if (basePointer >= baseCapacity)
				throw new IllegalStateException("Cannot copy edge refs into ch graph. " + "pointer:" + pointer
						+ ", cap:" + maxCapacity + ", basePtr:" + basePointer + ", baseCap:" + baseCapacity);
	...
	String toDetailsString() {
		return toString() + ", shortcuts:" + nf(shortcutCount) + ", nodesCH:(" + nodesCH.getCapacity() / Helper.MB
				+ "MB)";
	}
		...
		while (tmpIter.next()) {
			if (tmpIter.isShortcut() && tmpIter.getEdge() == edgeState.getEdge()) {
				...
				long edgePointer = tmpPrevEdge == EdgeIterator.NO_EDGE ? -1
						: isShortcut(tmpPrevEdge) ? chEdgeAccess.toPointer(tmpPrevEdge)
								: baseGraph.edgeAccess.toPointer(tmpPrevEdge);
				chEdgeAccess.internalEdgeDisconnect(edgeState.getEdge(), edgePointer, edgeState.getAdjNode(),
						edgeState.getBaseNode());
				break;
			}
	...
	final void setWeight(CommonEdgeIterator edge, double weight) {
		...
		long accessFlags = edge.getDirectFlags(-1) & scDirMask;
	...
	final double getWeight(CommonEdgeIterator edge) {
		...
		long flags32bit = edge.getDirectFlags(-1);
	...
	class CHEdgeIteratorImpl extends EdgeIterable implements CHEdgeExplorer, CHEdgeIterator {
		...
		public final long getFlags() {
			...
			return super.getDirectFlags(-1);
		}

		@Override
		public final long getFlags(int encoderIndex) {
			checkShortcut(false, "getFlags");
			return super.getDirectFlags(encoderIndex);
		}
		...
		public boolean isBackward(FlagEncoder encoder) {
			...
			if (isShortcut())
				return (getDirectFlags(encoder.getIndex()) & PrepareEncoder.getScBwdDir()) != 0;
		...
		public boolean isForward(FlagEncoder encoder) {
			assert encoder == weighting.getFlagEncoder() : encoder + " vs. " + weighting.getFlagEncoder();
			if (isShortcut())
				return (getDirectFlags(encoder.getIndex()) & PrepareEncoder.getScFwdDir()) != 0;
